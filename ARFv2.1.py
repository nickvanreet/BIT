...
def process_fasta_files(input_dir, target_sequence):
    ...
    for index, filename in enumerate(fasta_files, start=1):
        fasta_file_real = os.path.join(input_dir, filename)
        fasta_filename = os.path.basename(fasta_file_real)

        print(f"Processing FASTA {index}/{num_files}: {fasta_filename}")

        # Read the FASTA file and get the sequence length
        with open(fasta_file_real, 'r') as file:
            fasta_data = file.read().strip().split('\n', 1)
            sequence_length = len(fasta_data[1].replace('\n', ''))

        print(f"Sequence length: {sequence_length} bases")
        
        records = list(SeqIO.parse(fasta_file_real, "fasta"))

        if sequence_length < 380:
                # Triple the sequence
                for record in records:
                    sequence = record.seq
                    new_sequence = sequence * 3

                    # Update the record with the modified sequence
                    record.seq = new_sequence

                # Generate a new file name
                fasta_file = fasta_filename.replace(".fasta", "_triple.fasta")
                
                # Save the modified records to a new FASTA file
                with open(fasta_file, "w") as new_file:
                    SeqIO.write(records, new_file, "fasta")

                print(f"Modified sequence saved to {fasta_file}")

                # Remove the original small FASTA file
                os.remove(fasta_file_real)

        else:
                # Just go for TRF with a new file name
                for record in records:
                    sequence = record.seq
                ...
                # Generate a new file name
                fasta_file = fasta_filename.replace(".fasta", "_single.fasta")
